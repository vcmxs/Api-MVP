...
function ExPickerPanel({ open, onClose, onSelect, onDone, rightRem = 34 }: {
  ...
  return createPortal(
    <div className="fixed top-0 z-[199] flex h-screen w-full max-w-[16rem] sm:max-w-xs flex-col border-r border-white/[0.08] bg-[#0f1117] shadow-2xl transition-transform duration-300 ease-in-out"
...
function WorkoutPickerPanel({ open, onClose, onSelect, available, programFolderId, onWorkoutCreated }: {
  ...
  return createPortal(
    <>
      <div
        className="fixed top-0 z-[198] flex h-screen w-full max-w-[18rem] sm:max-w-sm flex-col bg-[#0d0f14] border-r border-white/[0.08] shadow-2xl transition-transform duration-300 ease-in-out"
        style={{ right: 0, transform: open ? "translateX(0)" : "translateX(100%)", pointerEvents: open ? "auto" : "none" }}
      >
        {/* ── List mode ── */}
        {mode === "list" && (
...
function AssignPlanModal({ plan, open, onOpenChange, onAssigned }: {
  ...
  useEffect(() => {
    ...
    const uniqueProgramIds = [...new Set(Object.values(plan.schedule).filter(Boolean).map(s => s!.programId).filter(id => !!id))]
