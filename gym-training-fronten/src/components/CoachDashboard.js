...
const CoachDashboard = ({ token, userId }) => {
  ...
  const submitPayment = async (traineeId) => {
    if (!paymentAmount) {
    ...
    try {
      ...
      // Refresh history
      const res = await axios.get(`${API_URL}/users/coaches/${userId}/trainees/${traineeId}/history`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      setPaymentHistory(res.data);
                ...
                return (
                  <React.Fragment key={trainee.id}>
                    <tr style={{ borderBottom: isExpanded ? 'none' : '1px solid var(--border-color)' }}>
                      ...
                      <td style={{ padding: '1rem' }}>
                        <button
                          onClick={() => toggleManageTrainee(trainee)}
                        ...
                        <td colSpan="4" style={{ padding: '1rem' }}>
                          <div style={{ display: 'flex', gap: '2rem', flexWrap: 'wrap' }}>
                            ...
                            <div style={{ flex: 1, minWidth: '300px' }}>
                              ...
                              <div style={{ maxHeight: '150px', overflowY: 'auto', border: '1px solid var(--border-color)', borderRadius: '5px' }}>
                                {paymentHistory.length === 0 ? (
                                  <p style={{ padding: '10px', color: 'var(--text-secondary)', fontStyle: 'italic' }}>No history found.</p>
                                ) : (
