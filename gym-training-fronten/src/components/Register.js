...
function Register({ onRegister, onToggle }) {
  ...
  const handleFileChange = (e) => {
    ...
    if (!allowed.includes(file.type)) { setError(t('registerPage.errorImageType')); return; }
    if (file.size > 5 * 1024 * 1024)  { setError(t('registerPage.errorImageSize')); return; }
    setProfilePic(file);
    setError('');
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    setError('');
    if (formData.password !== formData.confirmPassword) { setError(t('registerPage.errorPasswordMismatch')); return; }
    if (formData.password.length < 6)                   { setError(t('registerPage.errorPasswordShort'));    return; }
    setLoading(true);
    try {
      const data = new FormData();
      Object.entries(formData).forEach(([k, v]) => data.append(k, v));
      if (profilePic) data.append('profilePic', profilePic);
      const response = await axios.post(`${API_URL}/auth/register`, data, { headers: { 'Content-Type': 'multipart/form-data' } });
      onRegister(response.data.user, response.data.token);
    } catch (err) {
      setError(err.response?.data?.message || t('registerPage.errorGeneric'));
    } finally {
      setLoading(false);
    }
  };
  ...
  const inputStyle = {
    ...
    borderRadius: '10px', color: '#EDF2F7', fontSize: '14px',
    outline: 'none', transition: 'border-color 0.2s ease, background 0.2s ease',
    fontFamily: "'Inter', -apple-system, sans-serif",
  };
  const labelStyle = { display: 'block', fontSize: '11px', fontWeight: '600', color: '#8892A4', marginBottom: '5px', letterSpacing: '0.5px', textTransform: 'uppercase' };
  const onFocus = (e) => { e.target.style.borderColor = '#00C4FF'; e.target.style.background = 'rgba(0,196,255,0.04)'; };
  const onBlur  = (e) => { e.target.style.borderColor = 'rgba(255,255,255,0.10)'; e.target.style.background = 'rgba(255,255,255,0.04)'; };

  return (
    <div style={{ minHeight: '100vh', background: '#07090F', display: 'flex', alignItems: 'flex-start', justifyContent: 'center', padding: '40px 20px 60px', fontFamily: "'Inter', -apple-system, BlinkMacSystemFont, sans-serif" }}>
      <div style={{ position: 'fixed', top: '20%', left: '50%', transform: 'translate(-50%,-50%)', width: '600px', height: '600px', background: 'radial-gradient(circle, rgba(0,196,255,0.06) 0%, transparent 65%)', pointerEvents: 'none', borderRadius: '50%' }} />

      <div style={{ width: '100%', maxWidth: '480px', position: 'relative', zIndex: 1 }}>
        <div style={{ textAlign: 'center', marginBottom: '28px' }}>
          <div style={{ width: '52px', height: '52px', margin: '0 auto 14px', background: 'linear-gradient(135deg, #00C4FF, #8B5CF6)', borderRadius: '14px', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 8px 24px rgba(0,196,255,0.22)' }}>
        ...
        <div style={{ marginBottom: '20px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
            {[{ value: 'trainee', label: t('registerPage.roleTrain'), icon: '🏋️' }, { value: 'coach', label: t('registerPage.roleCoach'), icon: '📋' }].map(({ value, label, icon }) => (
              <label key={value} style={{ cursor: 'pointer' }}>
                <input type="radio" name="role" value={value} checked={formData.role === value} onChange={handleChange} style={{ position: 'absolute', opacity: 0, width: 0, height: 0 }} />
                <div style={{ padding: '14px', border: `1px solid ${formData.role === value ? '#00C4FF' : 'rgba(255,255,255,0.08)'}`, borderRadius: '12px', background: formData.role === value ? 'rgba(0,196,255,0.07)' : 'rgba(255,255,255,0.03)', textAlign: 'center', transition: 'all 0.2s ease', boxShadow: formData.role === value ? '0 0 0 3px rgba(0,196,255,0.10)' : 'none' }}>
        ...
        <div style={{ background: 'rgba(255,255,255,0.03)', backdropFilter: 'blur(20px)', WebkitBackdropFilter: 'blur(20px)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '20px', padding: '28px', boxShadow: '0 24px 64px rgba(0,0,0,0.5)' }}>
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
            ...
            <div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
                ...
                <input type="file" accept="image/jpeg,image/jpg,image/png,image/webp" onChange={handleFileChange} style={{ ...inputStyle, padding: '8px 12px', fontSize: '13px', cursor: 'pointer' }} />
              </div>
            </div>
            ...
            <button type="submit" disabled={loading} style={{ background: 'linear-gradient(135deg, #00C4FF 0%, #8B5CF6 100%)', color: '#fff', fontWeight: '700', padding: '13px', borderRadius: '10px', border: 'none', fontSize: '15px', cursor: loading ? 'wait' : 'pointer', marginTop: '4px', transition: 'opacity 0.2s, transform 0.2s', letterSpacing: '0.3px', boxShadow: '0 4px 16px rgba(0,196,255,0.22)', opacity: loading ? 0.7 : 1, fontFamily: "'Inter', -apple-system, sans-serif" }}
        ...
        <div style={{ textAlign: 'center', marginTop: '20px' }}>
          ...
          <button onClick={onToggle} style={{ background: 'none', border: 'none', color: '#00C4FF', fontWeight: '600', fontSize: '14px', cursor: 'pointer', padding: 0, fontFamily: "'Inter', -apple-system, sans-serif" }}>
