...
class User {
    ...
    static async findById(id) {
        const result = await pool.query(
            'SELECT id, name, email, role, age, sex, phone, gym, profile_pic_url, notes, height, weight, created_at, subscription_status, subscription_tier, referral_code, subscription_start_date, subscription_end_date FROM users WHERE id = $1',
            [id]
        );
        return result.rows[0];
    }
